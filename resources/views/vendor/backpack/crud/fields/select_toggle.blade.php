...
@include('crud::fields.inc.wrapper_start')
    ...
    @if( isset($field['options']) && is_array($field['options']) )
        ...
        <select class="form-control select2_from_array select2-search--inline" id="{{$field['name']}}_{{$optionPointer}}"
                name="{{$field['name']}}" data-field-toggle="{{ json_encode($field['hide_when']) }}" type="select">
...
{{--                           value="{{$value}}" {{$optionValue!=='' && $optionValue == $value ? ' checked': ''}}> {!! $label !!}--}}
{{--                </label>--}}
    ...
    @push('crud_fields_scripts')
        <script>
            jQuery(document).ready(function ($) {
                ...
                $('select[data-field-toggle]').on('change', function () {
                    return $toggle($(this));
                });
                $('select[data-field-toggle]:checked').each(function () {
                    return $toggle($(this));
                });
            });
        </script>
